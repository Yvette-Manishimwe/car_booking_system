...
app.get('/earnings', authenticateToken, (req, res) => {
  ...
  // Corrected Query to get the total earnings of the driver
  const earningsQuery = `
    SELECT 
      SUM(t.amount * b.booked_seats) AS totalEarnings
    FROM 
      trips t
    JOIN 
      bookings b ON t.id = b.trip_id
    JOIN 
      notifications n ON n.trip_id = t.id AND n.driver_id = b.driver_id
    WHERE 
      t.driver_id = ? 
      AND n.status = 'Confirmed'
  `;
  
  // Adjusting the query call to pass only one parameter
