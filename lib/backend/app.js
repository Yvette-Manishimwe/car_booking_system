...
app.post('/register', uploadProfilePicture.single('profile_picture'), async (req, res) => {
  ...
  if (!passwordRegex.test(password)) {
    return res.status(400).json({
      message:
        'Password must be at least 8 characters long, contain at least one uppercase letter, one number, and one special character (@, #, $, etc.)',
    });
  }
  ...
  if (!phoneRegex.test(phone)) {
    return res.status(400).json({
      message: 'Phone number must start with 078, 079, 072, or 073 and contain exactly 10 digits',
    });
  }
  ...
  db.query('SELECT * FROM users WHERE email = ?', [email], (err, result) => {
    ...
    bcrypt.hash(password, 10, (err, hashedPassword) => {
      ...
      const query =
        'INSERT INTO users (name, email, password, phone, category, profile_picture) VALUES (?, ?, ?, ?, ?, ?)';
...
// Get available trips for a driver (only future trips)
app.get('/available-trips', authenticateToken, (req, res) => {
  const sql = `
    ...
    AND trips.trip_time > NOW()  -- Ensure only future trips are shown
    ORDER BY trips.trip_time ASC  -- Sort by upcoming trips first
  `;
  ...
  db.query(sql, (err, result) => {
    ...
    if (result.length === 0) {
      return res.status(404).json({
        success: false,
        message: 'No available future trips found',
      });
    }
    res.json(result); // Return only upcoming trips
  });
});


app.post('/request_reset_password', (req, res) => {
  ...
  db.query(userQuery, [email], (err, results) => {
    ...
    `;
    db.query(otpQuery, [userId, otp, expiresAt], (err) => {
      if (err) {
        console.error('Failed to store OTP:', err); // Log error
        return res.status(500).json({ success: false, message: 'Failed to generate OTP', error: err.message });
      }

      // Send email with the OTP
      const transporter = nodemailer.createTransport({
        host: 'smtp.gmail.com', // Explicitly specify the Gmail SMTP server
        port: 587,  
        auth: {
          user: 'samuelmanirakiza211@gmail.com', // Your Gmail
          pass: 'thip xewh bsvd nqjo', // App Password (not your regular password)
        },
      });

      const mailOptions = {
        from: 'samuelmanirakiza211@gmail.com',
        to: email,
        subject: 'Password Reset Request',
        text: `Use this 6-digit Verification Code to reset your password: ${otp}\nThis Verification Code will expire in 15 minutes.`,
      };
...
app.post('/reset_password', (req, res) => {
  ...
  const otpQuery = `
    ...
    WHERE otp = ?
  `;
  db.query(otpQuery, [otp], (err, results) => {
    ...
    const { user_id: userId, expires_at: expiresAt } = results[0];
    ...
    bcrypt.hash(newPassword, 10, (err, hashedPassword) => {
      if (err) return res.status(500).json({ success: false, message: 'Password hashing failed', error: err });

      // Update password in the database
      const updatePasswordQuery = 'UPDATE users SET password = ? WHERE id = ?';
      db.query(updatePasswordQuery, [hashedPassword, userId], (err) => {
        if (err) return res.status(500).json({ success: false, message: 'Failed to update password', error: err });

        // Delete the OTP
        const deleteOtpQuery = 'DELETE FROM password_reset_tokens WHERE user_id = ?';
        db.query(deleteOtpQuery, [userId], (err) => {
          if (err) return res.status(500).json({ success: false, message: 'Failed to invalidate Verification code', error: err });
...
app.post('/upload_proof/:bookingId', uploadPayment.single('proofOfPayment'), async (req, res) => {
  ...
  try {
    ...
    const paymentRegex = /Your payment of\s*(\d+(?:\.\d{1,2})?)\s*RWF\s*to\s*Asifiwe\s*(\d+)\s*has been completed at\s*(\d{4}-\d{2}-\d{2}\s*\d{2}:\d{2}:\d{2})/i;
...
app.get('/available_drivers', (req, res) => {
  ...
  console.log('Querying for available drivers with:', { departure_location, destination });

  const query = `
    SELECT users.id, users.name, users.profile_picture, Trips.id AS trip_id, 
      Trips.plate_number, Trips.trip_time, Trips.available_seats, 
       Trips.destination, Trips.departure_location
    FROM users
    INNER JOIN Trips ON users.id = Trips.driver_id
    WHERE Trips.departure_location = ? 
      AND Trips.destination = ? 
      AND Trips.available_seats > 0
      AND (
        DATE(Trips.trip_time) > CURDATE()  -- Future dates
        OR (DATE(Trips.trip_time) = CURDATE() AND TIME(Trips.trip_time) > CURTIME())  -- Today but future time
      );
  `;
  ...
  db.query(query, [departure_location, destination], (err, results) => {
    ...
    if (results.length === 0) {
      console.log('No upcoming drivers found for the specified route. Checking alternatives.');
      ...
      const alternativeQuery = `
        ...
        WHERE Trips.available_seats > 0
          AND (
            DATE(Trips.trip_time) > CURDATE()  
            OR (DATE(Trips.trip_time) = CURDATE() AND TIME(Trips.trip_time) > CURTIME())
          )
      ...
      db.query(alternativeQuery, (err, alternativeResults) => {
        ...
        res.json({
          ...
          message: 'No available drivers for this route. Showing alternative drivers.',
        });
      });
    } else {
      res.json({
