...
app.post('/register', upload.single('profile_picture'), async (req, res) => {
  ...
  db.query('SELECT * FROM users WHERE email = ?', [email], (err, result) => {
    ...
    bcrypt.hash(password, 10, (err, hashedPassword) => {
      ...
      if (req.file) {
        profilePictureUrl = req.file.path; // Use the file path as the URL (or use a cloud storage service)
      }
...
app.get('/earnings', authenticateToken, (req, res) => {
  ...
  db.query(ratingQuery, [driver_id], (err, ratingResult) => {
    ...
    // Logic to check if the driver's rating is less than 3
    if (averageRating < 3) {
      // Logic to "pause" the driver
      // Example: Update driver status in the database to "paused" or "inactive"
      const pauseDriverQuery = 'UPDATE users SET status = "paused" WHERE id = ?';
      db.query(pauseDriverQuery, [driver_id], (err, result) => {
        if (err) {
          console.error('Error pausing driver:', err);
          return res.status(500).json({ error: 'Failed to pause driver' });
        }
        
        console.log('Driver paused due to low rating.');
        res.json({ message: 'Driver paused due to low rating', averageRating });
      });
    } else {
      // Return the average rating without pausing the driver
      res.json({ averageRating });
    }
  });
});

// Reminder functionality - Notify the driver if their rating is low
app.post('/send-reminder', authenticateToken, (req, res) => {
  ...
  db.query(ratingQuery, [driver_id], (err, ratingResult) => {
    ...
    // Check if the rating is less than 3 and send a reminder
    if (averageRating < 3) {
      // Example: Send a reminder via notification or message
      const reminderQuery = 'INSERT INTO notifications (driver_id, message, status) VALUES (?, ?, ?)';
      const reminderMessage = 'Reminder: You must end the week with a rating greater than 3 to avoid being paused.';
      
      db.query(reminderQuery, [driver_id, reminderMessage, 'Unseen'], (err, result) => {
        if (err) {
      ...
      res.json({ message: 'Rating is fine, no reminder needed', averageRating });
    }
  });
});
...
app.get('/available_drivers', (req, res) => {
  ...
  const query = `
    SELECT users.id, users.name, Trips.id AS trip_id, Trips.plate_number, Trips.trip_time, Trips.available_seats
    FROM users
  ...
  db.query(query, [departure_location, destination], (err, results) => {
    ...
    if (results.length === 0) {
      ...
      const alternativeQuery = `
        SELECT users.id, users.name, Trips.id AS trip_id, Trips.plate_number, Trips.trip_time, Trips.available_seats, ount
        FROM users
        INNER JOIN Trips ON users.id = Trips.driver_id
        WHERE Trips.available_seats > 0
...
app.get('/passenger-details', authenticateToken, (req, res) => {
  ...
  const sql = 'SELECT name, email,phone, category FROM users WHERE id = ?';
...
app.get('/driver-notifications', authenticateToken, (req, res) => {
  ...
  const query = `
    ...
    JOIN bookings b ON b.trip_id = t.id AND b.passenger_id = n.passenger_id
    WHERE n.driver_id = ?
